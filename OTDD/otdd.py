...
# sys.path.insert(1, "/home/jiashigao/code/OTDD/utils")
...
def get_class_matrix(
    x_labels, y_labels, class_distances, class_x_dict=None, class_y_dict=None
):

    if (class_x_dict is not None) and (
        not x_labels[0].item() == class_x_dict[x_labels[0].item()]
    ):
        x_labels = np.array([class_x_dict[i.item()] for i in x_labels])

    if (class_y_dict is not None) and (
        not y_labels[0].item() == class_y_dict[y_labels[0].item()]
    ):
...
class CostFunction:
    ...
    def distance(self, x_vals, y_vals, max_iter=None, mask_diagonal=False):
        ...
        Returns:
            ...
            M_dist - ndarray - (x_samples,y_samples) distance matrix
        """
    ...
    def label_distances(
        self, x_vals, y_vals, x_labels, y_labels, max_iter=None, gaussian=False
    ):
        """
    ...
    def distance_with_labels(
        ...
        class_distances, class_x_dict, class_y_dict = self.label_distances(
            x_vals,
            y_vals,
            x_labels,
            y_labels,
        ...
        if not gaussian_data_distance:
            ...
            dz = get_class_matrix(
                x_labels, y_labels, class_distances, class_x_dict, class_y_dict
            )
...
class EarthMoversCost(CostFunction):
    ...
    def cost_function(self, x_weights, y_weights, M_dist, max_iter):
        ...
        output = ot.emd2(
            x_weights, y_weights, M_dist, numItermax=max_iter, return_matrix=True
        )
...
class SinkhornCost(CostFunction):
    ...
    def __init__(
        self, distance_function, entropy, default_max_iter=1000, method="sinkhorn"
    ):
...
class RobustOTCost(CostFunction):
    ...
    def get_constraints(self, P, u, v, a_tilde, b_tilde, x_weights, y_weights):
        ...
        constraints.append(
            cp.sum(
                [
                    ((x_weights[i] - a_tilde[i]) ** 2) / x_weights[i]
        ...
        constraints.append(
            cp.sum(
                [
                    ((y_weights[i] - b_tilde[i]) ** 2) / y_weights[i]
                    for i in range(y_weights.shape[0])
                ]
            )
            <= self.rho
        )
