# from OTDD.otdd import DistanceFunction, CostFunction
from OTDD.otdd import *
import torch
...
class TensorDataset(Dataset):
    ...
    def subsample(self, total_size, equal_classes=True, classes_range=None):

        idxs = torch.arange(self.features.shape[0])
        classes = list(set(self.labels.numpy()))
        if classes_range != None:
            classes = classes[classes_range[0] : classes_range[1]]
        ...
        if equal_classes:
            ...
            for c in classes:
                sample_idx += list(
                    np.random.choice(
                        idxs[self.labels == c],
                        min(n_samp, (self.labels == c).sum().item()),
                        replace=False,
                    )
                )
        else:
            sample_idx = np.random.choice(
                idxs, min(total_size, self.features.shape[0]), replace=False
            )
...
class PytorchEuclideanDistance(PytorchDistanceFunction):
    ...
    def gaussian_distance_from_stats(self, mu_x, sigma_x, mu_y, sigma_y, eps=1e-6):
        ...
        cost = (
            gaussian_distance_from_stats(mu_x, sigma_x, mu_y, sigma_y, eps=1e-6) ** 0.5
        )
...
class SinkhornTensorized(KeopsRoutine):
    ...
    def __init__(self, cost, debias=True, blur=0.05, reach=None, scaling=0.5):
        super().__init__()
        ...
        assert (
            self.cost.backend == "tensorized"
        ), "use a distance function with a tensorized backend"
    ...
    def routine(
        ...
        **kwargs,
        ...
        if self.debias:
            C_xx = self.calculate_cost(
                x, x.detach(), class_xx, mask_diagonal=mask_diagonal
            )
            C_yy = self.calculate_cost(
                y, y.detach(), class_yy, mask_diagonal=mask_diagonal
            )

        else:
            C_xx, C_yy = None, None

        C_xy = self.calculate_cost(
            x, y.detach(), class_xy, mask_diagonal=mask_diagonal
        )  # (B, N, M)
        C_yx = self.calculate_cost(
            y, x.detach(), class_yx, mask_diagonal=mask_diagonal
        )  # (B, M, N)

        diameter, ε, ε_s, ρ = scaling_parameters(
            x, y, self.p, self.blur, self.reach, self.diameter, self.scaling
        )
...
class SinkhornOnline(KeopsRoutine):
    ...
    def __init__(
        ...
        assert (
            self.cost.backend == "online"
        ), "use a distance function with an online backend"

    def logconv(self, C, dtype):
        if len(C) == 2:
            D = C[0].shape[1]
            log_conv = generic_logsumexp(
                "( B - (P * " + self.cost.formula + " ) )",
                "A = Vi(1)",
                "X = Vi({})".format(D),
                "Y = Vj({})".format(D),
                "B = Vj(1)",
    ...
    def softmin_online(self, ε, C_xy, f_y, log_conv=None):
        ...
        if len(C_xy) == 2:
            ...
            return -ε * log_conv(
                x, y, f_y.view(-1, 1), torch.Tensor([1 / ε]).type_as(x)
            ).view(-1)
            ...
            return -ε * log_conv(
                x, y, Z, L, f_y.view(-1, 1), torch.Tensor([1 / ε]).type_as(x)
            ).view(-1)
    ...
    def routine(
        ...
        **kwargs,
        ...
        softmin = partial(
            self.softmin_online, log_conv=self.logconv(C_xy, dtype=str(x.dtype)[6:])
        )
...
class SamplesLossTensorized(CostFunction):
    ...
    def distance(
        self, x_vals, y_vals, mask_diagonal=False, max_iter=None, return_transport=False
    ):
        ...
        cost, coupling, C_xy, transport = self.keops_routine.routine(
            x_weights, x_vals, y_weights, y_vals, mask_diagonal=mask_diagonal
        )
    ...
    def distance_with_labels(
        ...
        class_vals_xy = self.label_distances(
            x_vals, y_vals, x_labels, y_labels, gaussian=gaussian_class_distance
        )
        ...
        if self.keops_routine.debias:
            class_vals_xx = self.label_distances(
                x_vals, x_vals, x_labels, x_labels, gaussian=gaussian_class_distance
            )

            class_vals_yy = self.label_distances(
                y_vals, y_vals, y_labels, y_labels, gaussian=gaussian_class_distance
            )
    ...
    def bootstrap_label_distance(
        ...
        for i in range(num_iterations):
            ...
            d_xy = torch.tensor(
                get_class_matrix(label_x, label_y, *class_vals_xy)
            ).type_as(x_vals)
            ...
            if self.keops_routine.debias:
                d_xx = torch.tensor(
                    get_class_matrix(label_x, label_x, *class_vals_xx)
                ).type_as(x_vals)
                d_yy = torch.tensor(
                    get_class_matrix(label_y, label_y, *class_vals_yy)
                ).type_as(x_vals)
            else:
                d_xx = None
                d_yy = None
