...
def plot_coupling(
    ...
    plt_im = tf.shade(
        ds.Canvas(plot_width=1200, plot_height=1200).raster(
            xr.DataArray(coupling * distance_matrix), interpolate="linear"
        ),
        cmap=cmap,
    ).data
...
def plot_class_distances(
    class_distances, row_classes, col_classes, cmap="OrRd", figsize=(8, 8), text=True
):
    ...
    ax.set_xticks(np.arange(class_distances.shape[1] + 1) - 0.5, minor=True)
    ax.set_yticks(np.arange(class_distances.shape[0] + 1) - 0.5, minor=True)
...
def plot_connection_graph_ds(node_df, edge_df):
    ...
    edge_plot = tf.shade(
        canvas.line(connect_edges(node_df, edge_df), "x", "y", agg=ds.count()),
        how="log",
        cmap=["#000000"],
        alpha=100,
    )
...
def plot_connection_graph_hv(node_df, edge_df):
    ...
    layout = (
        datashade(graph, normalization="log", cmap=["#000000"], alpha=150)
        * nodes.opts(size=5, color="cat", cmap="Category20")
...
def plot_distance_network(xs, ys, x_labels, y_labels, M_dist, plot_type="ds"):
    ...
    return plot_from_idxs(
        xs, ys, x_idxs, y_idxs, x_labels, y_labels, plot_type=plot_type
    )
...
def plot_network_threshold(
    xs, ys, x_labels, y_labels, M, thr, thr_type="greater", plot_type="ds"
):
...
def plot_network_k_connections(
    xs, ys, x_labels, y_labels, M, k, k_type="greater", plot_type="ds"
):
