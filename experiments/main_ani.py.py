...
def Animal_run(queried_set, Dc, X_test, y_test):
    q_dataset = pd.concat(queried_set)

    q_dataset_x = q_dataset[q_dataset.columns.difference(["label"])]
    q_dataset_y = q_dataset["label"]

    X_train, _, y_train, _ = train_test_split(
        Dc[Dc.columns.difference(["label"])],
        Dc["label"],
        test_size=0.01,
        random_state=42,
    )
    X_train = (
        torch.tensor(pd.concat([X_train, q_dataset_x]).values.astype(np.float64))
        .view((-1, 1, 32, 32))
        .type(torch.FloatTensor)
    )
    y_train = torch.tensor(
        pd.concat([y_train, q_dataset_y]).values.astype(np.float64)
    ).type(torch.FloatTensor)
    X_test = (
        torch.tensor(X_test.values.astype(np.float64))
        .view((-1, 1, 32, 32))
        .type(torch.FloatTensor)
    )
    y_test = torch.tensor(y_test.values.astype(np.float64)).type(torch.FloatTensor)

    AnimalModels.train(X_train, y_train)
    accuracy = AnimalModels.test(X_test, y_test)
    return accuracy
...
if __name__ == "__main__":
    ...
    all_animal = pd.concat(
        [animal_owned_real, pd.concat([data for data in animal_real.values()])]
    )

    _, X_test, _, y_test = train_test_split(
        all_animal[all_animal.columns.difference(["label"])],
        all_animal["label"],
        test_size=0.2,
        random_state=42,
    )
    ...
    for explore_rate in explore_rate_list:
        animal_owned = copy.deepcopy(animal_owned_real)
        animal = copy.deepcopy(animal_real)

        queried_set, remain_budget = exploration(
            datasets=animal,
            B=explore_rate * all_budget,
            predicate_list=predicate_set_animal,
        )
        utility_set = calculate_utility(queried_set, animal_owned)
        # score1 = SV(queried_set, animal_owned)
        score2 = UCB(queried_set, animal_owned, utility_set)
        ...
        accuracy = Animal_run(queried_set, animal_owned, X_test, y_test)
        accuracy_list.append(accuracy)
        np.save("animal_list_6.4_2.npy", accuracy_list)
    print(accuracy_list)
    np.save("animal_list_6.4_2.npy", accuracy_list)
    # transform = torchvision.transforms.Compose(
    #     [
