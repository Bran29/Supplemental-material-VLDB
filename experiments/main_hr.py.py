...
def prepare_dataset():
    ...
    # employee_owned = df_dataset
    employee_owned = consumer
...
def Employee_run(queried_set, Dc, X_test, y_test):
    q_dataset = pd.concat(queried_set)

    q_dataset_x = q_dataset[q_dataset.columns.difference(["is_promoted"])]
    q_dataset_y = q_dataset["is_promoted"]

    X_train, _, y_train, _ = train_test_split(
        Dc[Dc.columns.difference(["is_promoted"])],
        Dc["is_promoted"],
        test_size=0.001,
        random_state=42,
    )
    ...
    cluster_model = EmployeeModels.clust(X_train, y_train, X_test, y_test)
    accuracy = cluster_model.Kmeans()
    return accuracy
...
if __name__ == "__main__":
    ...
    all_employee = pd.concat(
        [employee_owned_real, pd.concat([data for data in employee_real.values()])]
    )

    _, X_test, _, y_test = train_test_split(
        all_employee[all_employee.columns.difference(["is_promoted"])],
        all_employee["is_promoted"],
        test_size=0.1,
        random_state=42,
    )
    ...
    for explore_rate in explore_rate_list:
        employee_owned = copy.deepcopy(employee_owned_real)
        employee = copy.deepcopy(employee_real)

        queried_set, remain_budget = exploration(
            datasets=employee,
            B=explore_rate * all_budget,
            predicate_list=predicate_set_employee,
        )
        utility_set = calculate_utility(queried_set, employee_owned)
        # score1 = SV(queried_set, employee_owned)
        score2 = UCB(queried_set, employee_owned, utility_set)
        ...
        accuracy = Employee_run(queried_set, employee_owned, X_test, y_test)
        accuracy_list.append(accuracy)
        np.save("employee_list_6.4.npy", accuracy_list)
    print(accuracy_list)
    np.save("employee_list_6.4.npy", accuracy_list)
    # transform = torchvision.transforms.Compose(
    #     [
